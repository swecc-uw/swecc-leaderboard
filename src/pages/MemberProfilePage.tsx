...
import { RawGithubStats, RawLeetcodeStats, Member } from '../types';
import {
  Box,
  Button,
  Container,
...
const Widgets: React.FC<{ member: Member }> = ({ member }) => {
  ...
  const borderColor = useColorModeValue('gray.200', 'gray.700');
  const [githubStats, setGithubStats] = useState<RawGithubStats>();
  const [leetcodeStats, setLeetcodeStats] = useState<RawLeetcodeStats>();
